...
// These are the BN254 Poseidon2 parameters used by the pinned Noir compiler. The
// 64 rounds consist of four full rounds, 56 partial rounds, and four full rounds.
const INTERNAL_DIAGONAL: [&str; 4] = [
...
const EXTERNAL_ROUND_CONSTANTS: [[&str; 4]; 8] = [
    [
...
const PARTIAL_ROUND_CONSTANTS: [&str; 56] = [
...
pub(crate) fn permutation(
    ...
    state = external_matrix(builder, field, &state);

    for constants in &EXTERNAL_ROUND_CONSTANTS[..4] {
        state = external_round(builder, field, state, constants);
    }
    for constant in PARTIAL_ROUND_CONSTANTS {
        state[0] = field.add(builder, &state[0], &constant_from_hex(builder, constant));
        state[0] = s_box(builder, field, &state[0]);
        state = internal_matrix(builder, field, &state);
    }
    for constants in &EXTERNAL_ROUND_CONSTANTS[4..] {
        state = external_round(builder, field, state, constants);
    }
...
fn external_round(
    ...
    for (value, constant) in state.iter_mut().zip(constants) {
        *value = field.add(builder, value, &constant_from_hex(builder, constant));
        *value = s_box(builder, field, value);
    }
    external_matrix(builder, field, &state)
}

fn s_box(builder: &CircuitBuilder, field: &PseudoMersennePrimeField, value: &BigUint) -> BigUint {
    let square = field.mul(builder, value, value);
    let fourth = field.mul(builder, &square, &square);
    field.mul(builder, &fourth, value)
}
...
fn external_matrix(
    ...
    let t0 = field.add(builder, &input[0], &input[1]);
    let t1 = field.add(builder, &input[2], &input[3]);
    let twice_b = field.add(builder, &input[1], &input[1]);
    let t2 = field.add(builder, &twice_b, &t1);
    let twice_d = field.add(builder, &input[3], &input[3]);
    let t3 = field.add(builder, &twice_d, &t0);
    let twice_t1 = field.add(builder, &t1, &t1);
    let four_t1 = field.add(builder, &twice_t1, &twice_t1);
    let t4 = field.add(builder, &four_t1, &t3);
    let twice_t0 = field.add(builder, &t0, &t0);
    let four_t0 = field.add(builder, &twice_t0, &twice_t0);
    let t5 = field.add(builder, &four_t0, &t2);
    let t6 = field.add(builder, &t3, &t5);
    let t7 = field.add(builder, &t2, &t4);
    [t6, t5, t7, t4]
}
...
fn internal_matrix(
    ...
    let sum = input.iter().skip(1).fold(input[0].clone(), |sum, value| {
        field.add(builder, &sum, value)
    });
    std::array::from_fn(|index| {
        let scaled = field.mul(
            builder,
            &input[index],
            &constant_from_hex(builder, INTERNAL_DIAGONAL[index]),
        );
        field.add(builder, &scaled, &sum)
    })
}

fn constant_from_hex(builder: &CircuitBuilder, value: &str) -> BigUint {
    let value = FieldElement::from_hex(value).expect("Poseidon2 constants are valid BN254 values");
